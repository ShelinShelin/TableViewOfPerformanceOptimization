    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        if (_isCancelled) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [self _asyncDidFinishedDisplayLayer:self finished:NO];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (_isCancelled) {
                ...
                [self _asyncDidFinishedDisplayLayer:self finished:YES];
            }
        });
    });
}
...
- (void)_asyncDisplayWithContext:(CGContextRef)context
    ...
    if ([self.delegate respondsToSelector:@selector(asyncDisplayWithContext:size:isCancelled:)]) {
        [self.delegate asyncDisplayWithContext:context size:size isCancelled:isCancelled];
    }
    
}

- (void)_asyncDidFinishedDisplayLayer:(CALayer *)layer
                            finished:(BOOL)finished {
    
    if ([self.delegate respondsToSelector:@selector(asyncDidFinishedDisplayLayer:finished:)]) {
        [self.delegate asyncDidFinishedDisplayLayer:layer finished:finished];
    }
}
